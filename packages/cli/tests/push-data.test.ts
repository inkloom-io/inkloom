...
type MockDataClient = ReturnType<typeof createMockDataClient>;
...
function createTmpDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "push-data-test-"));
}
...
describe("applyDiffData", () => {
  let client: MockDataClient;

  beforeEach(() => {
    client = createMockDataClient();
  });
  ...
  it("should create folders top-down and track returned IDs", async () => {
    ...
    const summary = await applyDiffData(
      client as unknown as Parameters<typeof applyDiffData>[0],
      diff,
      [],
      { branchId: "branch_1" }
    );
  ...
  it("should collect errors without stopping", async () => {
    ...
    const failClient = {
      ...client,
      createPage: async () => {
        throw new Error("Data mutation failed");
      },
      removePage: async () => {
    ...
    const summary = await applyDiffData(
      failClient as unknown as Parameters<typeof applyDiffData>[0],
      diff,
      [],
      { branchId: "branch_1" }
    );
    ...
    assert.ok(summary.errors[0].includes("Data mutation failed"));
...
describe("isCoreMode detection", () => {
  ...
  it("should detect core mode with --data-api-url", () => {
    const localOpts = { dataApiUrl: "https://test.d1.cloud" };
    const globalOpts = { json: false };
    // With dataApiUrl set, it's always core mode
    assert.equal(!!localOpts.dataApiUrl, true);
  });

  it("should detect core mode with env var and no token", () => {
    const hasD1Url = true;
    const hasToken = false;
    assert.equal(hasD1Url && !hasToken, true);
  });

  it("should use platform mode when token is present", () => {
    const hasD1Url = true;
    const hasToken = true;
    assert.equal(hasD1Url && !hasToken, false);
  });
});

// ---------------------------------------------------------------------------
// Tests: End-to-end with walkMdxFiles + computeDiff + applyDiffData
// ---------------------------------------------------------------------------
...
describe("end-to-end push flow (core mode)", () => {
  ...
  it("should push local MDX files to D1 with correct diff", async () => {
    // Create local files
    writeFile(
      "quickstart.mdx",
