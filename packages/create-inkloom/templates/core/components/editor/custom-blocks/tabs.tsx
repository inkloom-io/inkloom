    ...
    render: (props) => {
        ...
        const updateChildren = () => {
          ...
          setChildren(
            childBlocks.map((childBlock: any) => ({
              id: childBlock.id,
              title: (childBlock.props?.title as string) || "Tab",
      ...
      const addTab = useCallback(() => {
        ...
        (editor as any).insertBlocks(
          [{ type: "tab", props: { title: `Tab ${childBlocks.length + 1}` } }],
          insertAfterBlock,
          "after"
        );
        ...
        >
          <div className="bn-tabs-header">
            {hasChildren ? (
              <div className="bn-tabs-tab-list">
                {children.map((child: { id: string; title: string; icon: string }, index: number) => (
                  <button
                    key={child.id}
                    type="button"
              ...
              <>
                <span className="bn-tabs-label">Tabs</span>
                <button
                  type="button"
          ...
          {!hasChildren && (
            <p className="bn-tabs-hint">
              Add tabs to create a tabbed interface.
            </p>
          )}
        </div>
      );
