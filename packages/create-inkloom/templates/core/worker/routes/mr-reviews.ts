...
export function createMrReviewRoutes(authorize?: ProjectAuthorizer) {
    ...
    .post(
      ...
      async (context) => {
        ...
        await db.insert(mrReviewComments).values({
          ...
          threadId: thread.id,
          content: input.content,
          createdBy: effectiveUser(access, input.userId),
          createdAt: now,
          updatedAt: now,
          isEdited: false,
        });
    ...
    .post(
      ...
      async (context) => {
        ...
        await db
          ...
          .set({
            status: "resolved",
            resolvedBy: effectiveUser(access, context.req.valid("json").userId),
            resolvedAt: now,
            updatedAt: now,
          })
          .where(eq(mrReviewThreads.id, thread.id));
