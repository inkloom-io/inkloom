...
function InlineIconRenderer(props: any) {
  ...
  const handleIconChange = (newIcon: string | null) => {
    ...
    // We match on the current icon value. If multiple icons share the same
    // value we fall back to the first match, which is acceptable because
    // ProseMirror keeps the component instance stable per-node.
